...
prepare:
		@if [ -f Utilities/embedXcode_prepare ]; then Utilities/embedXcode_prepare $(PROJECT_FILE_PATH); rm -r Utilities/embedXcode_prepare; fi;
...
ifeq ($(UPLOADER),micronucleus)
		osascript -e 'tell application "System Events" to display dialog "Click OK and plug the Digispark into the USB port." buttons {"OK"} with icon caution with title "embedXcode"'
...
boards:
		...
		@if [ -d $(DIGISPARK_APP) ];  then echo "---- $(notdir $(basename $(DIGISPARK_APP))) ---- ";  \
			grep .name $(DIGISPARK_PATH)/hardware/tiny-digispark/boards.txt;  echo; fi
